...
async function main() {
  ...
  console.log("Upgrading to VendingMachineV4...");
  ...
  if (
    ...
    console.log(
      "This can happen if the bytecode of V4 is very similar to previous versions."
    );
  } else {
  ...
  // Try to call V4-specific functions to verify upgrade
  try {
    // Call the version function
    const version = await upgraded.version();
    console.log("Contract version:", version);

    // Try to access the V4-specific state variable
    try {
      const v4Variable = await upgraded.v4StateVariable();
      console.log("V4 State Variable:", v4Variable.toString());
      console.log(
        "✅✅ V4 state variable is accessible! Full upgrade confirmed!"
      );
    } catch (error) {
      console.error("❌ Error accessing V4 state variable:", error.message);
      console.log(
        "The upgrade may have only updated certain functions but not the storage layout."
      );
    }
  } catch (error) {
    console.error("❌ Error verifying upgrade:", error.message);
    console.error("The upgrade may not have been successful at all.");
  }
}
